...
struct PowerModeConfig: Codable, Identifiable, Equatable {
    ...
    init(from decoder: Decoder) throws {
        ...
        hasHotkey = try container.decodeIfPresent(Bool.self, forKey: .hasHotkey) ?? false
    ...
    func encode(to encoder: Encoder) throws {
        ...
        try container.encode(hasHotkey, forKey: .hasHotkey)
    }
...
class PowerModeManager: ObservableObject {
    ...
    func setHotkey(for configId: UUID, enabled: Bool) {
        if let index = configurations.firstIndex(where: { $0.id == configId }) {
            configurations[index].hasHotkey = enabled
            saveConfigurations()
            
            NSLog("🎯 PowerModeManager: Setting hotkey for \(configurations[index].name) (\(configId)) to \(enabled)")
            
            // Notify that hotkey configuration changed
            NotificationCenter.default.post(name: .powerModeHotkeyChanged, object: nil, userInfo: ["configId": configId, "enabled": enabled])
        }
    }
    
    func activatePowerMode(with configId: UUID) async {
        guard let config = getConfiguration(with: configId), config.isEnabled else { return }
