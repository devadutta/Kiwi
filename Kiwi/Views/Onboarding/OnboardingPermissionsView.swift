...
struct OnboardingPermissionsView: View {
    ...
    @State private var permissionTimers: [Timer] = []
    ...
    private let permissions: [OnboardingPermission] = [
        ...
        OnboardingPermission(
            ...
            description: "Select the audio input device you want to use with \(AppConfiguration.appName).",
            icon: "headphones",
            type: .audioDeviceSelection
        ),
        OnboardingPermission(
            title: "Accessibility Access",
            description: "Allow \(AppConfiguration.appName) to help you type anywhere in your Mac.",
            icon: "accessibility",
            type: .accessibility
        ),
        OnboardingPermission(
        ...
        OnboardingPermission(
            ...
            description: "Set up a keyboard shortcut to quickly access \(AppConfiguration.appName) from anywhere.",
            icon: "keyboard",
    ...
    var body: some View {
        ...
        .onDisappear {
            // Clean up any running permission timers to prevent CPU waste
            cleanupPermissionTimers()
        }
    }
    ...
    private func requestPermission() {
        ...
        case .accessibility:
            ...
            // Start checking for permission status with proper timer tracking
            let timer = Timer.scheduledTimer(withTimeInterval: 0.5, repeats: true) { timer in
                if AXIsProcessTrusted() {
                    timer.invalidate()
                    // Remove from tracked timers
                    permissionTimers.removeAll { $0 == timer }
                    permissionStates[currentPermissionIndex] = true
                    withAnimation {
                        showAnimation = true
                    }
    ...
    private func moveToNext() {
        if currentPermissionIndex < permissions.count - 1 {
            ...
            // Clean up timers before moving to model download
            cleanupPermissionTimers()
            withAnimation {
    ...
    /// Clean up all running permission timers to prevent CPU waste
    private func cleanupPermissionTimers() {
        for timer in permissionTimers {
            timer.invalidate()
        }
