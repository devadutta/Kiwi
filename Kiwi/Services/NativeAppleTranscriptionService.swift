...
class NativeAppleTranscriptionService: TranscriptionService {
    private let logger = Logger(subsystem: AppConfiguration.loggingSubsystem, category: "NativeAppleTranscriptionService")
    ...
    func transcribe(audioURL: URL, model: any TranscriptionModel) async throws -> String {
        ...
        try await ensureModelIsAvailable(for: transcriber as Any, locale: locale)
    ...
    private func ensureModelIsAvailable(for transcriber: Any, locale: Locale) async throws {
        #if canImport(Speech) && ENABLE_NATIVE_SPEECH_ANALYZER
        guard let speechTranscriber = transcriber as? SpeechTranscriber else {
            logger.error("Invalid transcriber type passed to ensureModelIsAvailable")
            return
        }
