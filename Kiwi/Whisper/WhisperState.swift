...
class WhisperState: NSObject, ObservableObject {
    ...
    // Transcription Services - Pre-initialized for faster first-time access
    private var localTranscriptionService: LocalTranscriptionService!
    private var cloudTranscriptionService: CloudTranscriptionService!
    private var nativeAppleTranscriptionService: NativeAppleTranscriptionService!
    ...
    let logger = Logger(subsystem: AppConfiguration.loggingSubsystem, category: "WhisperState")
    ...
    init(modelContext: ModelContext, enhancementService: AIEnhancementService? = nil) {
        ...
        let appSupportDirectory = FileManager.default.urls(for: .applicationSupportDirectory, in: .userDomainMask)[0]
            .appendingPathComponent(AppConfiguration.appSupportDirectoryName)
        ...
        // Pre-initialize transcription services to eliminate first-time delays
        self.cloudTranscriptionService = CloudTranscriptionService()
        self.nativeAppleTranscriptionService = NativeAppleTranscriptionService()
        self.parakeetTranscriptionService = ParakeetTranscriptionService(customModelsDirectory: self.parakeetModelsDirectory)
        ...
        // Pre-initialize window managers to eliminate first-time delay
        preInitializeWindowManagers()
    }
    ...
    private func preInitializeWindowManagers() {
        // Pre-create window managers to eliminate first-time delay
        logger.info("🚀 Pre-initializing window managers for faster hotkey response")
        
        // Create both window managers so they're ready regardless of recorder type
        miniWindowManager = MiniWindowManager(whisperState: self, recorder: recorder)
        notchWindowManager = NotchWindowManager(whisperState: self, recorder: recorder)
        
        logger.info("✅ Window managers pre-initialized successfully")
    }
    ...
    private func transcribeAudio(_ url: URL) async {
        ...
        // Play stop sound when transcription starts
        Task {
            await MainActor.run {
